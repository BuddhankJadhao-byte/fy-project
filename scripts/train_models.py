...
from src.data import iqr_clip, load_dataset
...
def main() -> None:
    ...
    parser.add_argument("--tune", action="store_true", help="Run time-series hyperparameter search.")
    args = parser.parse_args()
    config = load_config(args.config)
    ensure_output_directories(config)
    frame = iqr_clip(load_dataset(config["paths"]["processed_data"]), factor=config["training"]["iqr_factor"])
